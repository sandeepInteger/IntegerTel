...
const sections = [
  {
    id: "crash",
    label: "Crash & Stability",
    title: "Crash & Stability Testing",
    icon: CrashIcon,
    ...
    bullets: [
      "Field-based crash reproduction",
      "Failure mode identification",
    ],
    detail: "We replicate device crashes in real-world network environments — not just lab simulations. Our field engineers identify precise failure modes across different network conditions, carrier configurations, and geographic areas, providing reproducible test cases for engineering teams.",
  },
  {
    id: "modem",
    label: "Modem Performance",
    ...
    bullets: [
      "Registration & attach behavior",
      "Mobility & handover evaluations",
      "Throughput & signaling analysis",
    ],
    detail: "Deep modem-level performance analysis covering the full connection lifecycle — from initial attach and registration through handover events, mobility transitions, and throughput benchmarking across varied network loads and signal conditions.",
  },
  {
    id: "validation",
    label: "4G/5G Validation",
    title: "4G/5G Validation",
    icon: ValidationIcon,
    ...
    bullets: [
      "SA/NSA stability",
      "Carrier feature behavior",
    ],
    detail: "We validate device behavior across 5G Standalone and Non-Standalone architectures — testing feature flag compliance, carrier-specific configurations, and stability under real network conditions across multiple operators and frequency bands.",
  },
  {
    id: "drive",
    label: "Drive Testing",
    ...
    bullets: [
      "Urban, suburban & rural routes",
      "Multiband, multi-technology testing",
    ],
    detail: "Structured drive test campaigns across diverse geographies — urban dense, suburban, and rural environments — capturing real-world device behavior across frequency bands and network technologies to expose coverage and performance gaps.",
  },
  {
    id: "reporting",
    label: "Reporting",
    ...
    detail: "Every test engagement delivers structured, actionable reporting — parsed device logs with annotated issue timelines, categorized defect summaries by severity and frequency, and engineering-grade optimization recommendations ready for chipset and software teams.",
  },
];

const stats = [
  { value: "5G",   label: "SA & NSA Coverage"  },
  { value: "Multi", label: "Band & Technology"  },
  { value: "OEM",  label: "Grade Reporting"     },
];
  ...
  return (
    <div className="w-full">
      ...
      <div className="relative w-full bg-[#080f1e] overflow-hidden">
        ...
        {/* Violet-blue glow — chipset/silicon feel */}
        <div className="absolute inset-y-0 left-0 w-2/3 bg-[radial-gradient(ellipse_at_left_center,rgba(139,92,246,0.11),transparent_60%)] pointer-events-none" />

        {/* Animated circuit trace lines */}
        <div className="absolute inset-0 pointer-events-none overflow-hidden">
          <svg className="absolute inset-0 w-full h-full opacity-20" viewBox="0 0 1200 400" fill="none" preserveAspectRatio="xMidYMid slice">
            {/* Horizontal traces */}
            {[80, 160, 240, 310].map((y, i) => (
              <motion.line key={`h${i}`} x1="0" y1={y} x2="1200" y2={y}
                stroke="rgba(139,92,246,0.4)" strokeWidth="0.5"
            ...
            {/* Vertical traces */}
            {[200, 450, 680, 900].map((x, i) => (
              <motion.line key={`v${i}`} x1={x} y1="0" x2={x} y2="400"
            ...
            {/* Signal pulse dots on intersection */}
            {[[200, 80], [450, 160], [680, 240], [900, 310]].map(([x, y], i) => (
              <motion.circle key={`dot${i}`} cx={x} cy={y} r="3"
        ...
        <div className="absolute -right-32 top-1/2 -translate-y-1/2 pointer-events-none">
          <div className="w-[480px] h-[480px] rounded-full border border-violet-900/40" />
          <div className="absolute inset-[80px] rounded-full border border-violet-800/25" />
          <div className="absolute inset-[160px] rounded-full bg-violet-900/20" />
        </div>
        ...
        <div className="max-w-7xl mx-auto px-6 sm:px-10 py-16 sm:py-20 relative z-10">
          ...
          >
            <motion.div variants={fadeUp}>
              <span className="inline-flex items-center gap-2 bg-violet-500/15 border border-violet-500/25 text-violet-400 text-xs font-bold tracking-widest uppercase px-4 py-1.5 rounded-full">
            ...
            <motion.p variants={fadeUp} className="text-slate-400 text-base sm:text-lg leading-relaxed">
              Multi-region drive testing, modem performance validation, and advanced log analysis
              for next-gen mobile devices and chipset certification — in real network conditions.
            </motion.p>
            ...
            <motion.div variants={fadeUp} className="flex flex-wrap gap-2.5">
              {["5G SA/NSA", "Modem Validation", "Crash Reproduction", "Log Analysis", "Drive Testing"].map((t) => (
                <span key={t}
                  className="text-xs font-semibold text-violet-300 border border-violet-800/50 bg-violet-900/30 px-3 py-1.5 rounded-full">
                  {t}
                </span>
              ))}
            </motion.div>
      ...
      <div className="w-full bg-[#f8faff] relative overflow-hidden">
        ...
        <div className="absolute inset-0 pointer-events-none">
          ...
          <div className="absolute top-0 right-0 w-[500px] h-[400px] bg-[radial-gradient(ellipse_at_top_right,rgba(139,92,246,0.07),transparent_65%)]" />
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-5 sm:px-8 py-16">
              ...
              return (
                <button key={s.id} onClick={() => setActive(i)}
                  className={`flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-semibold border transition-all duration-200
                    ${isActive
                      ? "bg-white border-violet-200 text-violet-600 shadow-md shadow-violet-100/60"
                      : "bg-white/60 border-slate-200 text-slate-500 hover:border-violet-100 hover:text-slate-700 hover:bg-white"
                    }`}
                >
                  <span className={isActive ? "text-violet-500" : "text-slate-400"}><Icon /></span>
                  {s.label}
                  {isActive && <span className="w-1.5 h-1.5 rounded-full bg-violet-500 ml-0.5" />}
                </button>
              );
            })}
          </motion.div>
              ...
              <div className="lg:col-span-3 bg-white border border-slate-200 rounded-3xl p-8 shadow-sm relative overflow-hidden">
                ...
                <div className="relative z-10 flex flex-col gap-6">
                  <div className="flex items-center gap-2.5">
                    <span className="w-4 h-[2px] bg-violet-500 rounded-full" />
          ...
          >
            <div className="flex items-center gap-3 mb-8">
              <span className="w-5 h-[2px] bg-violet-500 rounded-full" />
              <span className="text-violet-600 text-xs font-bold tracking-widest uppercase">All Capabilities</span>
            </div>
                  ...
                  <motion.button
                    ...
                    className={`group text-left bg-white border rounded-2xl p-6 flex flex-col gap-4 transition-all duration-200 hover:-translate-y-0.5
                      ${isActive
                        ...
                        : "border-slate-200 shadow-sm hover:border-violet-100 hover:shadow-md"
                      }`}
                  >
          ...
          >
            <div className="flex flex-col gap-1 text-center sm:text-left">
              <p className="text-slate-900 font-extrabold text-lg">Ready to test your device in the field?</p>
              <p className="text-slate-400 text-sm">
                Our field engineers validate chipset and device performance across real carrier networks — from initial attach to full drive test campaigns.
              </p>
            </div>
            <div className="flex items-center gap-3 shrink-0">
              <a href="tel:+14699955509"
                className="text-sm font-semibold text-slate-600 border border-slate-200 px-5 py-2.5 rounded-full hover:border-violet-300 hover:text-violet-600 transition-all duration-200 flex items-center gap-1.5">
              ...
              <a href="#contact"
                className="text-sm font-bold text-white bg-violet-600 hover:bg-violet-500 px-6 py-2.5 rounded-full shadow-md shadow-violet-500/20 hover:shadow-violet-400/30 transition-all duration-200 flex items-center gap-2">
                Request a Proposal
                <ArrowIcon />
              </a>
            </div>
...
export default ChipsetTestingPage;