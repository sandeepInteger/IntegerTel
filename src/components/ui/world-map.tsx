...
interface MapProps {
  ...
  lineColor?: string;
  // ✅ New prop — pass the bg color to match your container
...
export default function WorldMap({
  ...
  dotColor,                           // ✅ auto-derived from bg if not passed
}: MapProps) {
  const svgRef = useRef<SVGSVGElement>(null);
  const map = new DottedMap({ height: 100, grid: "diagonal" });

  // Derive dot color from backgroundColor if not explicitly provided
  const resolvedDotColor = dotColor ?? (
    backgroundColor === "transparent" || backgroundColor === "black" || backgroundColor.startsWith("#0") || backgroundColor.startsWith("#1")
      ? "#FFFFFF30"   // light dots on dark bg
      : "#00000030"   // dark dots on light bg
  );
  ...
  const svgMap = map.getSVG({
    ...
    shape: "circle",
    backgroundColor: backgroundColor,  // ✅ no longer hardcoded white/black
  });
  ...
  return (
    // ✅ removed bg-white dark:bg-black — now fully transparent by default
